...
alter table public.integrations
  ...
  check (provider in ('google', 'notion', 'canvas'));
...
alter table public.source_snapshots
  drop constraint if exists source_snapshots_source_check;

alter table public.source_snapshots
  add constraint source_snapshots_source_check
  check (source in ('notion', 'gmail', 'caldav', 'google_calendar', 'manual', 'system', 'canvas'));

alter table public.source_files
  drop constraint if exists source_files_source_check;
...
as $$
  ...
  where integration_tokens.user_id = token_user_id
    ...
    and token_provider in ('google', 'notion', 'canvas')
  limit 1;
$$;
