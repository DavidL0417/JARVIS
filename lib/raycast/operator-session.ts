...
// Session-side operator gate for the hidden Raycast status card. The signed-in user
// must BE the operator (their Supabase id === RAYCAST_OPERATOR_USER_ID). Every other
...
// (lib/raycast/operator-auth.ts). Fails closed on every error.
export async function requireRaycastOperatorSession(): Promise<OperatorSessionResult> {
...
// Pure check for the dashboard payload's `isRaycastOperator` flag — does this user id
// match the configured operator? False when unset or mismatched. Never throws.
export function isRaycastOperator(userId: string | null | undefined): boolean {
  const operatorUserId = process.env.RAYCAST_OPERATOR_USER_ID?.trim()
