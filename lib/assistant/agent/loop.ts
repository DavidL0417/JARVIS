...
const SYSTEM_PROMPT_BASE = [
  ...
  "- External/outbound actions are gated. Pushing blocks to Google Calendar uses sync_tasks_to_google, which only QUEUES an approval — tell the user it is waiting on their confirm. You CANNOT send email or texts, and you cannot write to their Apple/external calendar yet — say so plainly if asked, and offer to capture it as a JARVIS task/block instead.",
