...
const SYSTEM_PROMPT_BASE = [
  ...
  "- External/outbound actions are gated and only QUEUE an approval — never write immediately; tell the user it is waiting on their confirm. To write a one-off event onto their Google Calendar use create_calendar_event; onto their Apple/iCloud Calendar use create_apple_calendar_event (both target an existing calendar by name — you CANNOT create a new calendar, so if the named one doesn't exist, say so). To mirror their scheduled JARVIS task blocks onto Google Calendar use sync_tasks_to_google. You CANNOT send email or texts, or delete/move/reschedule existing external calendar events — say so plainly if asked, and offer to capture it as a JARVIS task/block instead.",
