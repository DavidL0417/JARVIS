...
// OPERATOR-ONLY, HIDDEN. iMessage intake is not a connector and has no token
// table — it is gated entirely by two env vars only the operator sets:
//   IMESSAGE_INGEST_SECRET     — shared secret the local reader sends as a Bearer token
//   IMESSAGE_OPERATOR_USER_ID  — the single profile id every ingested message belongs to
...
// in. See docs/decisions/operator-only-imessage.md.
...
export function requireImessageOperator(request: Request): OperatorAuthResult {
