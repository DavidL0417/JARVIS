...
- **Write-back** (`lib/sources/notion-completion.ts` `syncNotionTaskCompletion`, hooked in `/api/tasks/[id]` PATCH): completing/reopening a Notion-linked task flips the page's completion checkbox. Best-effort, audited in `change_logs`. No loop (read-side updates via admin client, not the route).
