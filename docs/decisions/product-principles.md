...
- When Canvas access tokens are disabled, the internal Chrome extension acts as a read-only Canvas Academic Reader. JARVIS controls discovery/imports from the web app; the extension only executes read-only browser commands, reports live state, and sends selected snapshots/file metadata back.
...
- Canvas discovery should be granular and user-directed. Broad link discovery should start from Canvas All Courses, not an arbitrary active course page; the control page should present courses first, course-specific navigation tabs second, and tab items third. Discovery should not run LLM extraction. Model work is reserved for user-selected pages/files, and large files should be stored or recorded for later processing instead of blindly extracted during discovery.
